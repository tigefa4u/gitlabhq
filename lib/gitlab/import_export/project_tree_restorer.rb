      ...
      def create_relations
        project_relations_without_project_members.each do |relation_key, relation_definition|
          if relation_definition.present?
            create_sub_relations(relation_key.to_s, relation_definition, @tree_hash)
          elsif @tree_hash[relation_key.to_s].present?
            save_relation_hash(relation_key.to_s, @tree_hash[relation_key.to_s])
          end
        end
      ...
      def process_sub_relation(relation_key, relation_definition, relation_item)
        relation_definition.each do |sub_relation_key, sub_relation_definition|
          ...
          if sub_relation_definition.present?
            create_sub_relations(sub_relation_key.to_s, sub_relation_definition, relation_item, save: false)
          end

          # transform relation hash to actual object
          sub_relation_hash = relation_item[sub_relation_key.to_s]
          if sub_relation_hash.present?
            relation_item[sub_relation_key.to_s] = create_relation(sub_relation_key, sub_relation_hash)
          end
        end
      end
