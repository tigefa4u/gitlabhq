      ...
      describe "Timeout" do
        ...
        context 'when an invalid timeout was provided' do
          ...
          it 'raises an error for invalid number' do
            expect { builds }.to raise_error('jobs:deploy_to_production timeout is not a number')
          end
        end

        context 'when some valid timeout was provided' do
          before do
            config[:deploy_to_production][:timeout] = 90
          end

          it 'returns provided timeout value' do
            expect(builds.size).to eq(1)
            expect(builds.first[:options]).to include(job_timeout: 90)
          end
        end
      end
